...
I first read the README and features_info.txt to get an overview of the data. Next I looked at the files in a text editor. 
...
The number of rows in subject_test.txt and y_test.txt match the # of rows in X_test.txt. I was able to confirm that these files have the subject and test information for each row in x_test.txt
...
- activity_labels as activity_cd and activity
- features as feature_cd and feature
...
- convert "-"  and "," to "_"
- remove trailing "()"
- make unique by adding suffix of left 0 padded row numbers (these were stripped out later)
...
- alter to remove the _### required for uniqueness
...
- use dplyr's "arrange" and "group_by" with "summarise_each" create a relatively narrow and long dataset.
...
There were duplicate rows in the feature.txt file. It appears the original file was missing an X, Y or Z suffix on some entries. To preserve the original naming convention as much as possible, I worked around the bad entries by adding the row number to make them unique.  

Here are the non-unique entries in features.txt:
...
##Summary statistics of the tidy_narrow.txt file
|field          |summary stats    |
|---------------|-----------------|
...
##Description of the variables in the tidy_narrow.txt file
...
The file is 180 rows and 69 columns. Here are the results of running str() on it:
